...
namespace AirlineSystem.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Trips(int fromId, int toId, DateTime departureDate, int passengers)
        {
            ...
            if (toId == fromId)
            {
                ModelState.AddModelError("toId", "Аэропорт возврата должен отличаться от аэропорта выдачи заказов.");
            }

            if (departureDate < DateTime.Now)
            {
                ModelState.AddModelError("departureDate", "Эта поездка устарела");
            }
